    ...
    public partial class Interpreter
    {
        public static dynamic? ProcessFunc(Function func, List<dynamic> vals, string name, string startline)
        {
            ...
            for (int i = 0; i < p.Count; i++)
            {
                ...
                {
                    if (Globals.Vars[p[i]].RO == true || p[i] == "TextBGColor" || p[i] == "DLine" || p[i] == "NL" || p[i] == "BLANK" || p[i] == "COMMA" || p[i] == "COLON" || p[i] == "SC")
                    {
            ...
            for (int i = 0; i < lines.Count; i++)
            {
                ...
                else if (line.StartsWith("RemoveAtLIndex(") && line.EndsWith(")"))
                {
                    ...
                    if (VALS.Length != 2)
                    {
                        Console.WriteLine($"Need 2 parameters to remove list item, Line {line} Try going on https://github.com/ChristopherAliprantis/dinolang/wiki/ for help");
                        Environment.Exit(1);
                    }
                    ...
                    {
                        ((List<dynamic>)list).RemoveAt((int)index);
                    }
                    else
                    {
                ...
                else if (line.StartsWith("addtolist(") && line.EndsWith(")"))
                {
                    string[] VALS = line.Substring(10, line.Length - 11).Split(',');
                    if (VALS.Length != 2)
                    {
